...
INFO. The project consists of crafting the prototype of the lecture web for the web development course. Given the data which is covering the winter study material  main focus of the project was javascript but the project also included html and css.  We were supposed to employ various tools such as , `rollup`,`Babel`, `browser-sync`, `node-sass` `stylint` etc.
...
### Project set up
The project has `index.html` for the front page and `styles.css` for styles file. Files such as `package.json` and `package-lock.json` contain the information on the dependencies necessary to run the project (it is the one `npm install` refers to when installing dependencies). `lectures.json` then contain all the material that is displayed on the lecture pages.

Actual styling is developed inside `.scss`-files since it enables us higher flexibility and readability of the CSS code. `.scss`-files are for examples for header styling, lecturepage styling etc. Then the SCSS-files convert to CSS-files using `node-sass` package.

The project contains two `.html` pages stored in the main folder af the project. One `index.html` and one `fyrirlestur.html`. All the `.scss` files are stored in `/scr/styles` folder. All javascript files are stored in  `/scr/lib` except `index.js` which is stored in `/scr/`.


### Upplýsingar um alla sem unnu verkefni: 
Kjartan Elí Guðnason, Þorri Már Sigurþórsson and Dmitrii Fufachev

Original repo is from: https://github.com/vefforritun/vef1-2018-h2
